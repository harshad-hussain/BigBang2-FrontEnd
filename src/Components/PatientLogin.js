...
const PatientLogin = () => {
  ...
  function sendEmail(patientEmail) {
    ...
    setTimeout(() => {
      emailjs
        .send('service_t8kj37p', 'template_n0abj3s', templateParams, 'LoY459bOOZP_WbAfT')
        .then((response) => {
      ...
      <div className="container">
        <div className="form login">
          ...
          <form>
            ...
            <input type="button" className="button" onClick={handleFormSubmit} value="Login" />
          </form>
          <div className="signup">
            <span className="signup">
              Don't have an account? <Link to="/Patientreg">Signup</Link>
            </span>
          </div>
        </div>
